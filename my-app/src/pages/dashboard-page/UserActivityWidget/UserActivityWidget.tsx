...
const UserActivityWidget = ({ className }: IWidgetStandardProps) => {
  ...
  const subHeader: string | JSX.Element = isLoading ? (
    <Skeleton width="30%" />
  ) : (
    `${percentage}%`
  );
  ...
  async function setData() {
    ...
    const appLink =
      branding.urls.appDownload.url || "https://www.joinflow.com/apps/";

    const statistics: ILoginStatistics =
      await DashboardService.getLoginStatistics();
  ...
  return (
    <StandardWidget className={className}>
      <WidgetHeader header={userActivity} subHeader={subHeader} />
        ...
        <div className="logged-in-users">
          {isLoading ? (
            ...
            <span>
              ...
              {t("flowdashboard.statistics_widget.users_logged_in")}
            </span>
          )}
        </div>
