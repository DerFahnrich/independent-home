...
const UsersWidget = ({className}: IWidgetStandardProps): JSX.Element => {
    ...
    async function setData() {
        ...
        const extensionIds: number[] = extensions.map(extension => Number(extension.key.split("-")[1]));
        const licensesPerCountryKey = mapExtensionsToCountries(extensions);

        const countries = await UsersService.listCountries();
        const licensesPerCountry = populateLicenseCountryMap(countries, licensesPerCountryKey);

        const userRights = await UsersService.listUserRights(extensionIds, ["ADMIN"]);
    ...
    function mapExtensionsToCountries(extensions: AbbreviatedExtension[]): Map<string, number> {
    ...
    function populateLicenseCountryMap(countries: Country[], licensesPerCountryKey: Map<string, number>): LicensesCountry[] {
        ...
        countries.forEach(country => {
            ...
            const countryWithSameDescription = licensesPerCountry.find(c => c.country === country.description);
            if (countryWithSameDescription) {
                count += countryWithSameDescription.count;
                const sameDescIndex = licensesPerCountry.findIndex(c => c.country === country.description);
    ...
    async function populateRows(userRights: UserRight[], licensesPerCountry: LicensesCountry[]): Promise<Row[]> {
        const nbrAdmins = userRights.filter(right => right.userRights[0].active).length;
