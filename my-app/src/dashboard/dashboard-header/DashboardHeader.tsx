...
import {fetchNotifications} from "@store/dashboard-store/dashboardThunks";
...
} from "@store/dashboard-store/dashboardSlice"
...
const DashboardHeader = (): JSX.Element => {
    ...
    const [sessionContact, setSessionContact] = useState<ISessionContact>({} as ISessionContact);
    ...
    const fetchSessionContact = async (): Promise<void> => {
        const response: Response = await NetworkService.sendGet(URLToSessionContact);
    ...
    const handleOnClick = () => {
        const shouldToggleNotificationsList = notifications.length > 0 && !notificationsIsLoading;
    ...
    const renderCompanyName = (): JSX.Element => {
        if (customerIsLoading) {
            return <span className="company">{t("admin_dashboard.header.loading")}</span>
        }

        if (customer !== null) {
            return <span className="company">{customer.companyAddress.companyname}</span>
        }
    ...
    const renderNotificationsText = (): ReactNode => {
        ...
        if (showNotifications) {
            return <span className="notifications-text">{t("admin_dashboard.header.hide_notifications")}</span>
        }

        if (notifications.length > 0) {
            return <span className="notifications-text">{t("admin_dashboard.header.show_notifications")}</span>
        }

        return <span className="notifications-text">{t("admin_dashboard.header.no_notifications")}</span>
    };

    const renderWelcomeText = (): JSX.Element => {
        if (sessionContactIsLoading) {
            return <span className="welcome-text">{t("admin_dashboard.header.welcome_back")}...</span>
        }

        return (
            <span className="welcome-text">
                {t("admin_dashboard.header.welcome_back")} {sessionContact.extension.contact.firstName} {sessionContact.extension.contact.lastName}!
            </span>
        )
    ...
    return (
        <div className="dashboard-header">
            ...
            >
                <div className={`notifications-number-container ${showNotifications ? "hide" : ""}`}>
