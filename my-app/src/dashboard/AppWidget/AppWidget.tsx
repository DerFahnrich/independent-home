...
import WidgetLinksContent from "@commons/widget-content/WidgetLinksContent";
...
const AppWidget = ({className}: IWidgetStandardProps): JSX.Element => {
    ...
    useEffect(() => {
        AppsService.getAllAvailableApps().then( apps =>{
            const installed :Array<appStoreEntry> = [...apps.appStoreEntries].filter((app: appStoreEntry) => app.state === "installed");
            const available :Array<appStoreEntry> = [...apps.appStoreEntries].filter((app: appStoreEntry) => app.state === "available").slice(0,3);
            let displayApps;
            let linkPath;
